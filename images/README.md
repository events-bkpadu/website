...
Add your photographs to the matching category folder, or use `gallery/gallery.json` and run `node gallery/fetch-images.js`. When the server exposes directory listings, the website discovers the available files automatically with one directory request per category and does not probe missing filenames.
...
The downloader updates those generated fallback arrays after it downloads images. Normally edit only `gallery/gallery.json`.